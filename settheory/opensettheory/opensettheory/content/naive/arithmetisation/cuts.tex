...
% Section: Appendix: Cuts
%
\documentclass[../../../include/open-logic-section]{subfiles}

\begin{document}
	
	\olfileid{sfr}{arith}{cut}
\olsection{From $\Rat$ to $\Real$}\ollabel{cut}
...
\end{defn}\noindent 
So now we can say that $\sqrt{2} = \Setabs{p \in \Rat}{p^2 < 2\text{ or }p < 0}$. Of course, we need to check that this \emph{is} a cut, but I relegate that to \olref[sfr][arith][check]{checking}.
...
\begin{align*}
	...
	\end{cases}
\end{align*}
 We then need to check that each of these definitions always yields a cut. And finally, we need to go through an easy (but long-winded) demonstration that the cuts, so defined, behave exactly as they should. But I relegate all of this to \olref[sfr][arith][check]{checking}.
\end{document}