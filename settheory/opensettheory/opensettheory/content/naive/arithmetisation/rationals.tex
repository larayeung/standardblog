...
\olsection{From $\Int$ to $\Rat$}\label{s:ArithmetisationZQ}
...
As with the integers, we also want to define some basic operations. Where $\equivrep{i,j}{\Ratequiv}$ is the equivalence class under $\Ratequiv$ with $\tuple{ i, j}$ as !!a{element}, we say:
	...
	\end{align*}
We then need to check that these definitions behave as they \emph{ought} to; and I relegate this to \olref[sfr][arith][check]{checking}. But they indeed do!{} Finally, we want some way to treat integers \emph{as} rationals; so for each $i \in \Int$, we stipulate that $i_\Rat = \equivrep{i, 1_\Int}{\Ratequiv}$. Again, we check that all of this behaves correctly in \olref[sfr][arith][check]{checking}.
\begin{prob}
