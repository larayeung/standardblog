...
\begin{document}
		\olfileid{sfr}{ord-arithmetic}{ord-expo}
\olsection{Ordinal exponentiation}\ollabel{ord-expo}
...
\begin{prob}
	Using Transfinite Induction, prove that, if we define $\ordexpo{\alpha}{\beta} = \ordtype{\text{finfun}(\alpha, \beta), \sqsubset}$, we obtain the recursion equations of \olref[sfr][ord-arithmetic][ord-expo]{ordexporecursion}.
\end{prob}
\end{document}