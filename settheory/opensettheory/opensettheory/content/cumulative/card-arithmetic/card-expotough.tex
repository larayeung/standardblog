...
\begin{document}
		\olfileid{sfr}{card-arithmetic}{card-expotough}
\olsection{Some simplification with cardinal exponentiation}\ollabel{card-expotough}
Whilst defining $\canonord$ was a little involved, the upshot is a useful result concerning cardinal addition and multiplication, \olref[sfr][card-arithmetic][card-simp]{cardplustimesmax}. Transfinite exponentiation, however, cannot be simplified so straightforwardly. To explain why, I start with a result which extends a familiar pattern from the finitary case (though its proof  at quite a high level of abstraction):
...
\begin{proof}
	...
	\end{align*}
\end{proof}\noindent
We should not really expect to be able to simplify this any further, since $\cardfont{b} < \cardexpo{2}{\cardfont{b}}$ by \olref[sfr][card-arithmetic][card-opps]{lem:SizePowerset2Exp}. However, this does not tell us what to say about $\cardexpo{\cardfont{a}}{\cardfont{b}}$ when $\cardfont{b} < \cardfont{a}$. Of course, if $\cardfont{b}$ is \emph{finite}, we know what to do. 
\begin{prop}If $\cardfont{a}$ is infinite and $n \in \omega$ then $\cardexpo{\cardfont{a}}{n} = \cardfont{a}$
\end{prop}
\begin{proof}
	$\cardexpo{\cardfont{a}}{n} = \cardfont{a} \cardtimes \cardfont{a} \cardtimes \ldots \cardtimes \cardfont{a} = \cardfont{a}$, by $n-1$ applications of \olref[sfr][card-arithmetic][card-simp]{cardplustimesmax}. 
\end{proof}\noindent
...
\end{prop}
\begin{proof}
	$\cardexpo{2}{\cardfont{b}}\leq \cardexpo{\cardfont{a}}{\cardfont{b}} \leq \cardexpo{(\cardexpo{2}{\cardfont{b}})}{\cardfont{b}} = \cardexpo{2}{\cardfont{b}\cardtimes\cardfont{b}} = \cardexpo{2}{\cardfont{b}}$, reasoning as in \olref{cardexpo2reduct}.
\end{proof}\noindent
