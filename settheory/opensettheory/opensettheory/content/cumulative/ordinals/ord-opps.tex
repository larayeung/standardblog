...
\begin{document}
	\olfileid{sfr}{ordinals}{ord-opps}
	\olsection{Successor and limit ordinals}\ollabel{ord-opps}
...
\end{prop}
\begin{proof}
	Trivially, $\alpha \in \alpha \cup \{\alpha\} = \ordsucc{\alpha}$. Equally, $\ordsucc{\alpha}$ is a transitive set of ordinals, and hence an ordinal by \olref[sfr][ordinals][ord-basic]{corordtransitiveord}. And it is impossible that $\alpha \in \beta \in \ordsucc{\alpha}$, since then either $\beta \in \alpha$ or $\beta = \alpha$, contradicting \olref[sfr][ordinals][ord-basic]{ordordered}.
\end{proof}\noindent
This also licenses a variant of proof by transfinite induction:
\begin{thm}[Simple Transfinite Induction]\ollabel{simpletransrecursion} Let $\phi(x)$ be a formula such that:\footnote{The formula may have parameters.} 
	\begin{enumerate}
...
\end{prop}
\begin{proof}
	Let $Y = \Setabs{\ordsucc{\alpha}}{\alpha \in X}$, so that $\supstrict(X) = \bigcup Y$. Since ordinals are transitive and every member of an ordinal is an ordinal, $\supstrict(X)$ is a transitive set of ordinals, and so is an ordinal by \olref[sfr][ordinals][ord-basic]{corordtransitiveord}. 
