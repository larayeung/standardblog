...
But hold on!{} Why assume that there \emph{are} any stages after $M$? If there are none, then our justification will fail. So, to justify Pairs, we will have to add another principle to the story we told in \olref[sfr][z][z-story]{z-story}, namely:
\begin{enumerate}
...
\begin{prop}\ollabel{prop:pairsconsequences}For any sets $a$ and $b$, the following sets exist:
		\begin{enumerate}
...
\end{prop}
\begin{proof}
	\olref{singleton}. By Pairs, $\{a, a\}$ exists, which is $\{a\}$ by Extensionality. 
	
	\olref{binunion}. By Pairs, $\{a, b\}$ exists. Now $a \cup b = \bigcup \{a, b\}$ exists by Union. 

	\olref{tuples}. By \olref{singleton}, $\{a\}$ exists. By Pairs, $\{a, b\}$ exists. Now $\{\{a\}, \{a, b\}\} = \tuple{a, b}$ exists, by Pairs again.
\end{proof}
...
\end{prob}
%\begin{proof}
%	By two applications of Pairs, $\{\{a_1, a_2\}, \{a_1, a_3\}\}$ exists. By Union and Extensionality, $\bigcup \{\{a_1, a_2\}, \{a_1, a_3\}\} = \{a_1, a_2, a_3\}$ exists. Repeat this trick as often as necessary.
%\end{proof}
\end{document}