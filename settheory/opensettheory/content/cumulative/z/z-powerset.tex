...
\end{prop}
\begin{proof}
	The set $\Pow{\Pow{A \cup B}}$ exists by Powersets and \olref[sfr][z][pairs]{prop:pairsconsequences}. So by Separation, this set exists:
		$$C = \Setabs{z \in \Pow{\Pow{A \cup B}}}{(\exists x \in A)(\exists y \in B) z = \tuple{x, y}}$$
	Now, for any $x \in A$ and $y \in B$, the set $\tuple{x, y}$ exists by \olref[sfr][z][pairs]{prop:pairsconsequences}. Moreover, since $x, y \in A \cup B$, we have that $\{x\}, \{x, y\} \in \Pow{A \cup B}$, and $\tuple{x,y} \in \Pow{\Pow{A \cup B}}$. So $A \times B = C$.
\end{proof}\noindent
	%
