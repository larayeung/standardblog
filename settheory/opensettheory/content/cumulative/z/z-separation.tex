...
\begin{axiom*}[Scheme of Separation] For every formula $\phi(x)$,\footnote{Which may have parameters; for an explanation of what this means, see the discussion immediately after \olref[sfr][infinite][dedekind]{natinductionschema}.} this is an axiom: for any $A$, the set $\Setabs{x \in A}{\phi(x)}$ exists.
\end{axiom*}\noindent 
...
\begin{thm}\ollabel{thm:NoUniversalSet}There is no \emph{universal} set, i.e.\ $\Setabs{x}{x = x}$ does not exist.
\end{thm}
\begin{proof}
...
\begin{prop}\ollabel{prop:intersectionsexist}If $A \neq \emptyset$, then $\bigcap A = \Setabs{x}{(\forall y \in A)x \in y}$ exists.
	...
	\end{proof}
\end{prop}\noindent
Note the condition that $A \neq \emptyset$, though; for $\bigcap \emptyset$ would be the universal set, vacuously, contradicting \olref{thm:NoUniversalSet}.
