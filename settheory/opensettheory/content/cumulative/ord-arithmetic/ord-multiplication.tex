...
\begin{document}
		\olfileid{sfr}{ord-arithmetic}{ord-multiplication}
\olsection{Ordinal multiplication}\ollabel{ord-multiplication}
...
\begin{lem}\ollabel{ordtimeslessiswo}$\tuple{\alpha \times \beta, \rlexless}$ is a well-order, for any ordinals $\alpha$ and $\beta$.
\end{lem}
\begin{proof}
	Exactly as for \olref[sfr][ord-arithmetic][ord-addition]{ordsumlessiswo}.
\end{proof}\noindent 
And it is also not hard to prove that multiplication behaves thus:
\begin{lem}\ollabel{ordtimesrecursion} For any ordinals $\alpha, \beta$:
	\begin{align*}
		\alpha \ordtimes 0 &= 0\\
...
\begin{enumerate}
	...
	\item\ollabel{ordtimes3}  $\alpha \ordtimes (\beta \ordtimes \gamma) = (\alpha \ordtimes \beta) \ordtimes \gamma$
	\item\ollabel{ordtimes4}  If $\alpha \leq \beta$, then $\alpha \ordtimes \gamma \leq \beta \ordtimes \gamma$
	\item\ollabel{ordtimes5}  $\alpha \ordtimes (\beta \ordplus \gamma) = (\alpha \ordtimes \beta )\ordplus (\alpha\ordtimes \gamma)$
\end{enumerate}
\end{lem}
\begin{proof}
	Left as an exercise.
\end{proof}\noindent
You can prove (or look up) other results, to your heart's content. But, given \olref[sfr][ord-arithmetic][ord-addition]{ordsumnotcommute}, the following should not come as a surprise:
\begin{prop}
...
\begin{prob}
	Prove \olref[sfr][ord-arithmetic][ord-multiplication]{ordtimeslessiswo}, \olref[sfr][ord-arithmetic][ord-multiplication]{ordtimesrecursion}, and \olref[sfr][ord-arithmetic][ord-multiplication]{ordinalmultiplicationisnice}
\end{prob}
