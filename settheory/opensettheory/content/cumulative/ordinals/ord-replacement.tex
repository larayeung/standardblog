...
\begin{document}
		\olfileid{sfr}{ordinals}{ord-replacement}	
\olsection{Replacement}\ollabel{ord-replacement}
In \olref[sfr][ordinals][vn]{vn}, I motivated the introduction of ordinals by suggesting that we could treat them as order-types, i.e.\ canonical proxies for well-orderings. In order for that to work, we would need to prove that \emph{every well-ordering is isomorphic to some ordinal}. This would allow us to define $\ordtype{A, <}$ as the ordinal $\alpha$ such that $\tuple{A, <} \isomorphic \alpha$. 

Unfortunately, we \emph{cannot} prove the desired result only the Axioms we provided in earlier chapters. (We will see why in \olref[sfr][replacement][strength]{strength}, but trust me for now: we can't.) We need a new thought, and here it is:
\begin{axiom*}[Scheme of Replacement] For any formula $\phi(x, y)$,\footnote{Which may have parameters.} this is an axiom: 
