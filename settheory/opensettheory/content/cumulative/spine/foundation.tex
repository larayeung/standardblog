...
\begin{document}
	\olfileid{sfr}{spine}{foundation}
	\olsection{Foundation}\ollabel{foundation}
...
Now, there is a fairly straightforward (mathematical) sense in which we don't \emph{care} whether there are sets outside the hierarchy. (If there are any there, we can simply ignore them.) But we have motivated our \emph{concept} of set with the thought that every set is formed at some stage (see \stageshier{} in \olref[sfr][z][z-story]{z-story}.) So we will want to preclude the possibility of sets which fall outside of the hierarchy. Accordingly, we must add a new axiom, which ensures that every set occurs somewhere in the hierarchy. 
...
\end{proof}
\begin{lem}\ollabel{lem:TransitiveWellFounded} If $A$ is a transitive set, then there is some $\alpha$ such that $A \subseteq V_\alpha$.
\end{lem}
\begin{proof}
	Recalling the definition of ``$\supstrict(X)$'' from \olref[sfr][ordinals][ord-opps]{defsupstrict}, define:
	\begin{align*}
	...
	\end{align*}
	Suppose $D = \emptyset$. So if $x \in A$, then there is some $\delta \in \alpha$ such that $x \subseteq V_\delta$, so $x \in V_\alpha$ by \olref[sfr][spine][Valphabasic]{Valphabasicprops}. Hence $A \subseteq V_{\alpha}$, as required. 
