...
\begin{document}
	\olfileid{sfr}{cardinals}{card-fix}
	\olsection{Appendix: $\aleph$-fixed-points}\ollabel{card-fix}
...
\begin{proof}
	...
	\end{align*}
	Now $\kappa$ is a cardinal by \olref[sfr][card-arithmetic][card-classing]{unioncardinalscardinal}. But now:
	\begin{align*}
		\kappa = \bigcup_{n < \omega} \kappa_{n+1} = \bigcup_{n < \omega}\aleph_{\kappa_n} = \bigcup_{\alpha < \kappa}\aleph_\alpha = \aleph_\kappa 
	\end{align*}
%	By construction, $\kappa$ is the least cardinal greater than each $\kappa_n$. So $\aleph_\kappa$ is the least cardinal greater than each $\aleph_{\kappa_n}$, and hence greater than each $\kappa_{n+1}$. But equally $\kappa$ is the least cardinal greater than each $\kappa_{n+1} = \aleph_{\kappa_n}$. So $\kappa = \aleph_\kappa$.
\end{proof}\noindent
...
\begin{prop}\ollabel{bethfixed}There is a $\beth$-fixed-point, i.e.\ a $\kappa$ such that $\kappa = \beth_\kappa$
	\end{prop}
\begin{proof}
	As in \olref{alephfixed}, using ``$\beth$'' in place of ``$\aleph$''. 
\end{proof}
\begin{prop}\ollabel{stagesize}$\cardof{V_{\omega+\alpha}} = \beth_{\alpha}$. If $\omega \ordtimes \omega \leq \alpha$, then $\cardof{V_\alpha} = \beth_\alpha$.
\end{prop}
\begin{proof}
...
\end{cor}
\begin{proof}
	Let $\kappa$ be a $\beth$-fixed point, as given by \olref{bethfixed}. Clearly $\omega \ordtimes \omega < \kappa$. So $\cardof{V_\kappa} = \beth_\kappa = \kappa$ by \olref{stagesize}.
\end{proof}\noindent
...
We define a term $\tau$ as follows. For any $A$, let $\tau_0(A) = \cardof{A}$, let $\tau_{n+1}(A) = \beth_{\kappa_n}$, and let $\tau(A) = \bigcup_{n < \omega}\kappa_n$. As in \olref{bethfixed}, $\tau(A)$ is a $\beth$-fixed point for any $A$, and trivially $\cardof{A} < \tau(A)$. So now consider this recursive definition of $\daleth$-fixed-points:\footnote{I'm using the Hebrew letter ``$\daleth$''; so far as I know it has no standard definition in set theory.}
\begin{align*}
	\daleth_0 &= 0\\
