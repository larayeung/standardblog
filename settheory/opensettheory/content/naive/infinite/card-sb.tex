...
\begin{document}
	\olfileid{sfr}{cardinals}{card-sb}
	\olsection{Appendix: a proof of Schr\"oder--Bernstein}\ollabel{card-sb}
Before we depart from na\"ive set theory, we have one last na\"ive (but sophisticated\bang) proof to consider. This is a proof of Schr\"oder--Bernstein (\olref[sfr][siz][sb]{schroderbernstein}): if $\cardle{A}{B}$ and $\cardle{B}{A}$ then $\cardeq{A}{B}$; i.e.\ given !!{injection}s $f \colon A \to B$ and $g \colon B \to A$ there is !!a{bijection} $h \colon A \to B$. 
...
Using similar notation as \olref[sfr][infinite][dedekind]{Closure}, let
\begin{align*}
...
\begin{prop}\ollabel{Closureprops}
	...
	\begin{enumerate}
		...
		\item\ollabel{Closuresmallest} if $X$ is $f$-closed and $B \subseteq X$, then $\Closureofunder{f}{B} \subseteq X$.
	\end{enumerate}
\end{prop}
\begin{proof}
...
\begin{proof}
	...
	I first claim that if $x \in F$ but $y\notin F$ then $g(x) \neq g(y)$. For reductio suppose otherwise, so that $y = g(y) = g(x) = f(x)$. Since $x \in F$ and $F$ is $f$-closed by \olref{Closureprops}, we have $y = f(x) \in F$, a contradiction. 
